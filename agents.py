...
def check_relevancy(user_prompt: str, context: str, agent_output: str) -> dict:
    """
    Checks if the agent's output is relevant to the user prompt and context.
    
    Args:
        user_prompt: The original user query or task
        context: Available context (research findings, previous messages, etc.)
    ...
    """
    prompt = (
        "You are a relevancy checker. Your task is to determine if the agent's output "
        "is relevant and faithful to the user prompt and available context.\n\n"
        "=== USER PROMPT ===\n"
        f"{user_prompt}\n\n"
        "=== CONTEXT ===\n"
        f"{context[:2000] if context else 'No additional context available'}\n\n"
        "=== AGENT OUTPUT ===\n"
        f"{agent_output[:1500]}\n\n"
        "=== INSTRUCTIONS ===\n"
        "Evaluate whether the agent output:\n"
        "1. Directly addresses the user prompt\n"
        "2. Does not contain hallucinated or unsupported information\n"
        "3. Stays on-topic and is coherent with the context\n\n"
    ...
    try:
        ...
        # Log the relevancy check with confidence
        status = "RELEVANT" if decision.is_relevant else " NOT RELEVANT"
        conf_str = f"[{decision.confidence:.2f}]" if hasattr(decision, 'confidence') else "[N/A]"
        print(f"[RELEVANCY CHECK] {status} {conf_str} - {decision.reasoning[:100]}...")
        ...
        # Default to assuming relevant if check fails
        return {
            "is_relevant": True,
...
def create_reviewer_agent():
    ...
    This agent acts as a wrapper that calls the check_relevancy tool.
    """
    def reviewer_invoke(state, agent_output: str, agent_name: str) -> dict:
        """
        ...
        # Add critique notes if available
        critique = state.get("critique_notes", "")
        if critique:
        ...
        print(f"\n=== REVIEWER checking {agent_name} output ===")
        
        # Call the relevancy checking tool
        result = check_relevancy(user_prompt, context, agent_output)
        
        # Log detailed information
        print(f"[REVIEWER] Agent: {agent_name}")
        print(f"[REVIEWER] User Prompt: {user_prompt[:100]}...")
        print(f"[REVIEWER] Output Length: {len(agent_output)} chars")
        print(f"[REVIEWER] Relevancy: {'PASS' if result['is_relevant'] else 'FAIL'}")
