...
# Set LLM_PROVIDER=ollama to use Ollama (default), or "litellm" for university server
LLM_PROVIDER = os.getenv("LLM_PROVIDER", "ollama").lower()

if LLM_PROVIDER == "ollama":
    # Ollama local server configuration (OpenAI-compatible API)
    BASE_URL = os.getenv("OLLAMA_BASE_URL", "http://localhost:11434/v1")
    API_KEY = os.getenv("OLLAMA_API_KEY", "ollama")  # Ollama doesn't require a real API key
    MODEL_NAME = os.getenv("MODEL_NAME", "qwen2.5:14b")
else:
    # Legacy university LiteLLM configuration
...
def check_relevancy(user_prompt: str, context: str, agent_output: str, mode: str = "content") -> dict:
    """
    Checks if the agent's output is relevant to the user prompt and context.
    
    Args:
        user_prompt: The original user query or task
        context: Available context (workflow state or content depending on mode)
        agent_output: The output generated by the agent
        mode: "content" for output agents, "orchestration" for routing agents
    
    Returns:
        dict with 'is_relevant' (bool), 'reasoning' (str), 'confidence' (float)
    """
    if mode == "orchestration":
        if "Intent:" in agent_output and "Answer Mode:" in agent_output:
            # Router evaluation — judge intent classification only
            prompt = (
                "You are a routing decision reviewer. The router agent "
                ...
                f"{agent_output}\n\n"
                "=== INSTRUCTIONS ===\n"
                "Evaluate whether the intent classification is correct:\n"
                "1. Does the intent (code/research/quiz/general) match the query?\n"
                "2. Does the answer mode (short/long) suit the complexity?\n"
                "Respond with ONLY a JSON object:\n"
                '{"is_relevant": true/false, "confidence": 0.0-1.0, '
                '"reasoning": "brief explanation"}\n'
            )
        else:
            # Supervisor/critiquer — judge against workflow state
            prompt = (
                "You are a workflow decision reviewer. Determine if the "
                "agent's decision logically follows from the current state.\n\n"
                "=== WORKFLOW STATE ===\n"
                f"{context[:2000] if context else 'No state available'}\n\n"
                "=== AGENT DECISION ===\n"
                f"{agent_output[:1500]}\n\n"
                "=== INSTRUCTIONS ===\n"
                "Evaluate whether the decision:\n"
                "1. Is a valid next step given the workflow state\n"
                "2. Is consistent with what has been done so far\n"
                "3. Makes logical sense as an orchestration choice\n\n"
                "Respond with ONLY a JSON object:\n"
                '{"is_relevant": true/false, "confidence": 0.0-1.0, '
                '"reasoning": "brief explanation"}\n'
            )
    ...
    try:
        ...
        decision = RelevancyDecision.model_validate(data)

        status = "RELEVANT" if decision.is_relevant else "NOT RELEVANT"
        print(f"[RELEVANCY CHECK] {status} [{decision.confidence:.2f}] ({mode}) - {decision.reasoning[:100]}")
        ...
        print(f"[RELEVANCY CHECK] Error during validation: {e}")
        return {
            "is_relevant": True,
...
def create_reviewer_agent():
    """
    Creates a reviewer agent that checks the relevancy of agent outputs.
    Routes between content mode and orchestration mode based on agent type.
    """
    def reviewer_invoke(state, agent_output: str, agent_name: str, mode: str = "content") -> dict:
        """
        Review the output of an agent for relevancy.

        Args:
            state: The current ChatState
            agent_output: The output generated by the agent being reviewed
            agent_name: Name of the agent that produced the output
            mode: "content" or "orchestration"

        Returns:
            dict with relevancy check results
        """
        user_prompt = state.get("main_task", "")

        if mode == "orchestration":
            # For orchestration agents, context is the workflow state
            context_parts = [
                f"Has research: {bool(state.get('research_findings', []))}",
                f"Has draft: {bool(state.get('draft', '').strip())}",
                f"Critique notes: {state.get('critique_notes', 'none') or 'none'}",
                f"Revision number: {state.get('revision_number', 0)}",
                f"Intent classified as: {state.get('intent', 'unknown')}",
                f"Current sub-task: {state.get('current_sub_task', 'none') or 'none'}",
            ]
            context = "\n".join(context_parts)
        else:
            # For content agents, context is accumulated content
            context_parts = []
            findings = state.get("research_findings", [])
            if findings:
            ...
            critique = state.get("critique_notes", "")
            if critique:
                context_parts.append(f"Critique Notes:\n{critique}")
            code_snippet = state.get("code_snippet", "")
            if code_snippet:
                context_parts.append(f"Code Context:\n{code_snippet[:500]}")
            quiz_output = state.get("quiz_output", "")
            if quiz_output:
                context_parts.append(f"Quiz Output:\n{quiz_output[:500]}")
            context = "\n\n".join(context_parts) if context_parts else "No additional context available"

        print(f"\n=== REVIEWER checking {agent_name} output ({mode} mode) ===")

        result = check_relevancy(user_prompt, context, agent_output, mode=mode)

        print(f"[REVIEWER] Agent: {agent_name} | Mode: {mode}")
        print(f"[REVIEWER] User Prompt: {user_prompt[:100]}")
        print(f"[REVIEWER] Relevancy: {'PASS' if result['is_relevant'] else 'FAIL'} | Confidence: {result['confidence']:.2f}")
        print(f"[REVIEWER] Reasoning: {result['reasoning']}")
        ...
        return {
            ...
            "output_preview": agent_output[:200] + "..." if len(agent_output) > 200 else agent_output
        }
